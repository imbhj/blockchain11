...
const Baseball = () => {
  ...
  const [allNfts, setAllNfts] = useState<
    { tokenId: string; image: string; owner: string }[]
  >([]);
    ...
    try {
      ...
      for (let id = 0; id < maxTokenId; id++) {
        try {
          ...
          const owner = await baseballNftTokenContract.methods
            .ownerOf(id)
            .call();
          if (owner.toLowerCase() === account.toLowerCase()) {
            // ipfs://bafybeif4zdkotbumzuh6pxv5fjcvn3f4ajsqn74kiggelsskk6t5iqjsse/0.json
            const uri = await baseballNftTokenContract.methods
              .tokenURI(id)
              .call();
  ...
  const loadAllNFTs = async () => {
    ...
    try {
      const total = await baseballNftTokenContract.methods
        .getAllTokenIds()
        .call();
  ...
  const approveAllTokens = async () => {
    ...
    try {
      const isApproved = await baseballNftTokenContract.methods
        ...
        .call();
      if (isApproved == true) {
        alert("이미 모든 NFT를 판매 등록했습니다.");
        return;
      }
      await baseballNftTokenContract.methods
        .setApprovalForAll(baseballNftTokenAddress, true)
        .send({ from: account });
      alert("모든 NFT 판매 등록 완료");
    } catch (error) {
      console.log(`모든 NFT 판매 등록 실패 : ${error}`);
    }
  };
  ...
  return (
    <div className="App">
      ...
      <div>
        ...
        <button onClick={approveAllTokens}>모두 판매</button>
        {myNfts.length === 0 ? (
          <p>보유한 NFT가 없습니다.</p>
        ) : (
          myNfts.map((nft) => (
      ...
      <div>
        <h3>NFT 목록</h3>
        <button onClick={loadAllNFTs} style={{ marginTop: "10px" }}>
          이미지 가져오기
        </button>
        {allNfts.length === 0 ? (
          <p>NFT 없음</p>
          ...
          allNfts.map((nft) => (
            <div key={nft.tokenId}>
              ...
              {nft.owner.toLowerCase() !== account.toLowerCase() && (
                <button onClick={() => purchaseNFT(nft.tokenId)}>
                  🛒 구매하기
                </button>
              )}
            </div>
